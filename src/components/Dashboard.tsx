...
export default function Dashboard({
  ...
  async function openDoc(id: string) {
    ...
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : "Gagal membuka dokumen.",
      );
    } finally {
      setBusyId(null);
    }
  ...
  async function duplicateDoc(id: string) {
    ...
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : "Gagal menduplikat dokumen.",
      );
    } finally {
      setBusyId(null);
    }
  ...
  async function deleteDoc(id: string) {
    ...
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : "Gagal menghapus dokumen.",
      );
    } finally {
      setBusyId(null);
    }
      ...
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Kelola dokumen tersimpan &amp; template Anda.
          </p>
        </div>
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center">
                    <div className="flex flex-col items-center gap-2 text-slate-400">
                      ...
                      <p className="text-xs">
                        Belum ada dokumen. Klik <b>Buat Dokumen Baru</b> untuk
                        mulai.
                      </p>
                    </div>
                  </td>
                ...
                docs.map((d) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-xs text-slate-500">
                      {DOC_TYPES[d.doc_type]?.label.split(" / ")[0] ??
                        d.doc_type}
                    </td>
                    <td className="px-4 py-3">
      ...
      <div className="mt-8">
        ...
        {!loading && templates.length === 0 ? (
          <p className="rounded-xl border border-dashed border-border bg-white px-4 py-6 text-center text-xs text-slate-400">
            Belum ada template. Buka sebuah dokumen lalu klik{" "}
            <b>Simpan sebagai Template</b> di editor untuk menyimpan konfigurasi
            (perusahaan, pajak, terms, dll).
          </p>
        ) : (
