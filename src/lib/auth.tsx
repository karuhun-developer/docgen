...
import { getSession, setSession, subscribe, type AuthUser, type Session } from './session'
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      if (hash.includes('access_token')) {
        ...
        const dest = window.location.pathname === '/auth/callback' ? '/' : window.location.pathname
        window.history.replaceState({}, '', dest)
        if (at && rt) {
          setSession({ access_token: at, refresh_token: rt, user: { id: '', email: '' } })
        }
      }
  ...
  const register = useCallback(async (email: string, password: string) => {
    const res = await api.post<Partial<AuthResponse> & { needsConfirmation?: boolean }>(
      '/api/auth/register',
      { email, password },
    )
  ...
  return (
    <AuthContext.Provider value={{ user, ready, login, register, loginGoogle, logout }}>
      {children}
