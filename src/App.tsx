...
export default function App() {
  ...
  function handleReset() {
    if (
      confirm("Reset semua isian ke contoh awal? Data saat ini akan hilang.")
    ) {
      clearData();
      setData(defaultData());
      setCurrentDocId(null);
  ...
  async function saveTemplate(name: string) {
    ...
    try {
      ...
      alert(
        err instanceof ApiError ? err.message : "Gagal menyimpan template.",
      );
    }
  }
  ...
  saveShortcutRef.current = () => {
    if (!inEditor || locked || saving !== null || showAuth || showTemplate)
      return;
                ...
                {locked ? (
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">
                      {saving ? "Membuka…" : "Kembalikan ke Draft"}
                    </span>
                  </Button>
                ) : (
                  <Button
      ...
      {inEditor ? (
        locked ? (
          ...
          <main className="mx-auto grid w-full min-h-0 max-w-[1400px] flex-1 grid-cols-1 gap-6 px-4 sm:px-6 lg:grid-rows-1 lg:grid-cols-[minmax(380px,440px)_1fr]">
            <div
              className={`${tab === "edit" ? "block" : "hidden"} min-h-0 overflow-y-auto py-6 pr-1 lg:block`}
            >
            ...
            <div
              className={`${tab === "preview" ? "block" : "hidden"} min-h-0 overflow-y-auto py-6 lg:block`}
            >
  ...
  return (
    <main className="mx-auto min-h-0 w-full max-w-[900px] flex-1 overflow-y-auto px-4 py-6 sm:px-6">
      <div className="no-print mb-4 flex items-center gap-2 rounded-xl border border-accent/30 bg-accent/5 px-4 py-3 text-xs text-accent">
        ...
        Dokumen ini <b>Final</b> dan terkunci. Klik <b>Kembalikan ke Draft</b>{" "}
        di header untuk mengedit.
      </div>
      <div className="rounded-xl border border-border bg-slate-100 p-4 sm:p-6">
        <PreviewPane ref={printRef} data={data} />
      </div>
    ...
    <Modal open={open} onClose={onClose} title="Simpan sebagai Template">
      <form onSubmit={submit} className="space-y-3">
        <p className="text-xs text-slate-500">
          Template menyimpan seluruh konfigurasi dokumen ini (perusahaan,
          template visual, pajak, terms, tanda tangan, dll) agar bisa dipakai
          ulang untuk dokumen baru.
        </p>
        <div>
